...
export class Validators {
  ...
  static validateEmail(email: string): ValidationResult {
    ...
    // Comprehensive email regex - more strict
  ...
  // Phone number validation (Israeli format) - comprehensive
  static validatePhone(phone: string): ValidationResult {
    ...
    const mobilePattern = /^05[0-9]\d{7}$/; // 050-9999999 (10 digits)
    const landlinePattern = /^0[2-4,8-9]\d{7,8}$/; // 02-9999999, 03-9999999, etc. (9-10 digits)
    const internationalPattern = /^972[2-9]\d{7,8}$/; // +972-x-xxxxxxx
    ...
    // Additional validation for landline numbers
  ...
  // Student ID validation - more strict
  ...
  // Employee ID validation - more comprehensive
  static validateEmployeeId(employeeId: string): ValidationResult {
    ...
    // Validate numeric range (reasonable employee numbers)
  ...
  // Course code validation - more strict
  static validateCourseCode(courseCode: string): ValidationResult {
    ...
    // Common subject abbreviations validation (warn for uncommon ones)
    const commonSubjects = [
      'CS', 'CSE', 'IT', 'IS', // Computer Science
      'MATH', 'STAT', 'CALC', // Mathematics
      'PHYS', 'CHEM', 'BIO', // Sciences
      'ENG', 'HEB', 'ARAB', // Languages
      'ECON', 'BUS', 'MGT', 'FIN', // Business
      'PSY', 'SOC', 'PHIL', 'POL', // Social Sciences
      'LAW', 'HIST', 'GEO', // Humanities
      'ART', 'MUS', 'LIT', 'THR' // Arts
    ];
  ...
  // Academic level/stage validation (Bachelor's, Master's, PhD)
  ...
  // Study year validation (1st year, 2nd year, etc.)
...
export class UniquenessValidator {
  // Check if ID is unique within a specific entity type
  static async validateIdUniqueness(
    id: string, 
    entityType: 'students' | 'lecturers' | 'admins',
    excludeId?: string // For editing existing records
    ...
    try {
      ...
      switch (entityType) {
        ...
        case 'lecturers':
          ...
          break;
        // Add admins case when we implement it
        default:
          return { isValid: true };
      }

      const isDuplicate = existingRecords.some(record => {
        // Check the appropriate ID field based on entity type
      ...
      return { isValid: true }; // Allow on error to avoid blocking
    }
  }

  // Check national ID uniqueness within a specific entity type only
...
export class FormValidator {
  static async validateStudentForm(formData: {
    ...
    // Validate national ID (תעודת זהות) - now required
    if (!formData.national_id || formData.national_id.trim() === '') {
      ...
      if (!nationalIdResult.isValid) {
        ...
        // Check uniqueness within students table only
  ...
  static async validateLecturerForm(formData: {
    ...
    // Validate national ID (תעודת זהות) if provided
    if (formData.national_id) {
      const nationalIdResult = Validators.validateIsraeliId(formData.national_id);
      if (!nationalIdResult.isValid) {
        errors.national_id = nationalIdResult.error!;
      } else {
        // Check uniqueness within lecturers table only
        const nationalIdUniquenessResult = await UniquenessValidator.validateNationalIdUniqueness(
